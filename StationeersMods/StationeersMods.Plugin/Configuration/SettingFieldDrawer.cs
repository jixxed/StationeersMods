...
namespace StationeersMods.Plugin.Configuration
{
    internal class SettingFieldDrawer
    {
        ...
        public SettingFieldDrawer(float yMax, int rightColumnWidth)
        {
            _yMax = yMax;
        ...
        public void DrawSettingValue(SettingEntryBase setting)
        {
            ...
            else if (setting.CustomHotkeyDrawer != null)
            {
                ...
                DrawUnknownField(setting, _rightColumnWidth);
        ...
        private static void DrawListField(SettingEntryBase setting)
        {
            ...
            if (setting.SettingType == typeof(KeyCode))
                ...
                DrawComboboxField(setting, acceptableValues, _yMax);
        }
        ...
        private static void DrawEnumField(SettingEntryBase setting)
        {
            if (setting.SettingType.GetCustomAttributes(typeof(FlagsAttribute), false).Any())
                DrawFlagsField(setting, Enum.GetValues(setting.SettingType), _rightColumnWidth - 50);
            else
                DrawComboboxField(setting, Enum.GetValues(setting.SettingType), _yMax);
        }
        ...
        private static void DrawKeyCode(SettingEntryBase setting)
        {
            if (ReferenceEquals(_currentKeyboardShortcutToSet, setting))
            {
                ...
                if (_keysToCheck == null) _keysToCheck = input.SupportedKeyCodes.Except(new[] { KeyCode.Mouse0, KeyCode.None , KeyCode.Escape }).ToArray();
                ...
                GUILayout.BeginHorizontal();
                {
                    ...
                    if (GUI.changed && colorStr != newColorStr)
                    {
                        ...
                        else
                        {
                            ...
                            if (split.Length == 4 && float.TryParse(split[0], out var r) && float.TryParse(split[1], out var g) && float.TryParse(split[2], out var b) &&
