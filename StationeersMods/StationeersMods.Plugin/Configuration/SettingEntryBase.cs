...
namespace StationeersMods.Plugin.Configuration
{
    ...
    public abstract class SettingEntryBase
    {
        ...
        /// </summary>
        public ModVersionInfo ModVersionInfo { get; private set; }
        ...
        internal void SetFromAttributes(object[] attribs, ModVersionInfo modVersionInfo)
        {
                    ...
                    default:
                        ...
                        if (attrType.Name == "ConfigurationManagerAttributes")
                        {
                            ...
                            foreach (var propertyPair in _myProperties.Join(otherFields, my => my.Name, other => other.Name, (my, other) => new { my, other }))
                            {
                                ...
                                {
                                    Debug.LogWarning($"Failed to copy value {propertyPair.my.Name} from provided tag object {attrType.FullName} - " + ex.Message);
                                }
                            }
                            break;
                        }
