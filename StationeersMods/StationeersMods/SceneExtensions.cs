...
namespace StationeersMods
{
    ...
    public static class SceneExtensions
    {
        ...
        {
            if (self.isLoaded && self.IsValid()) SceneManager.UnloadSceneAsync(self.name);
        }
        ...
        {
            if (!self.isLoaded || !self.IsValid()) return default(T);
        ...
        {
            if (!self.isLoaded || !self.IsValid()) return null;
        ...
        public static T[] GetComponentsInScene<T>(this Scene self)
        {
            ...
            if (!self.isLoaded || !self.IsValid()) return components.ToArray();

            foreach (var go in self.GetRootGameObjects()) components.AddRange(go.GetComponentsInChildren<T>(true));
        ...
        public static GameObject Instantiate(this Scene self, GameObject original)
        {
            ...
            if (!self.isLoaded || !self.IsValid()) return o;
